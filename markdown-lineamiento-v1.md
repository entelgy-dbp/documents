...
4. Cumplimiento de puertos y adaptadores
...
2. Application como capa de orquestación
...
3. Infraestructura solo implementa puertos y adapta protocolos
...
4. Inbound adapters invocan únicamente input ports
  - No llaman servicios concretos
  - No acceden directamente al dominio

5. Output ports solo se implementan en infraestructura
  - Por ejemplo:
    - clientes HTTP
...
1. La interfaz se define en domain/repository
2. application/port/out crea un puerto de salida que extiende esa interfaz
3. Infraestructura implementa siempre el puerto, no el repositorio del dominio
...
1. domain/ — núcleo de negocio puro
  - Contiene:
...
2. application/ — orquestación y puertos hexagonales
  - Contiene:
    - Casos de uso (application/usecase).
...
3. infrastructure/ — adaptadores y detalles técnicos
  - Contiene:
...
4. config/ — configuración del runtime
...
5. route/ — rutas Camel (cuando aplica)
...
6. cross/ — componentes transversales específicos del microservicio
...
**Lineamientos**
1. JWT obligatorio en servicios expuestos
...
2. Configuración centralizada de validación JWT
...
3. Autorización basada en roles/grupos
...
4. Fachada de seguridad obligatoria en el framework
...
5. Integración con trazabilidad y logging
...
6. Reglas mínimas de claims del token
  - Obligatorios:
...
**Lineamientos**
1. Prohibición explícita de secretos en código/repositorio
  - No se permite:
...
2. Uso obligatorio de Google Secret Manager (GSM)
...
3. Uso de Google KMS para claves de cifrado
...
**Lineamientos**
1. mTLS obligatorio entre servicios internos
...
4. Compatibilidad con observabilidad y trazabilidad
...
1. Servicios REST (interfaces.rest)
...
2. Rutas Camel con entrada HTTP
...
3. Integraciones EDA (Kafka/PubSub/RabbitMQ)
...
4. Servicios internos “sólo backplane”
...
**Lineamientos**
1. Cada request HTTP entrante debe tener un contexto de trazabilidad compuesto al menos por:
  - traceId (OpenTelemetry – W3C Trace Context).
  - correlationId (X-Correlation-Id).
  - businessId (X-Business-Id, opcional según el caso de negocio).

2. El framework provee un filtro estándar TraceFilter (en observability-starter) que:
...
3. Todos los servicios REST generados por el arquetipo deben registrar TraceFilter como @Provider con prioridad alta (AUTHENTICATION/SECURITY), para que:

  - El MDC esté poblado antes de ejecutar controladores y lógica.
  - Los logs de aplicación y de Camel compartan los mismos IDs.

4. El traceId de OpenTelemetry es la referencia única para correlación:
...
5. Ningún servicio debe “inventar” su propio esquema de IDs de trazabilidad;
...
1. Toda llamada HTTP saliente (MicroProfile REST Client / quarkus-rest-client ) debe propagar el contexto de trazabilidad y negocio actual:
  - X-Correlation-Id.
  - X-Business-Id.
...
2. El framework provee HeadersPropagationFilter (en observability-starter) implementando ClientHeadersFactory de MicroProfile REST Client, que:
...
3. Cada cliente REST generado por el arquetipo debe declarar en application.properties:
```java
...
4. La propagación de traceparent/tracestate la gestiona OpenTelemetry; los equipos no deben implementarla manualmente. Siempre se utiliza:
...
1. Todos los servicios del framework deben:
...
2. El módulo de logging-starter define categorías y modelos:
  - LogCategories
    - Constantes:
      - TECH, BUSINESS, AUDIT.
  - AuditEvent
...
3. Campos mínimos que deben aparecer en cada log relevante
...
4. Buenas prácticas por categoría:
  - TECH
...
**Lineamientos**
1. Todos los errores expuestos por servicios REST del framework deben respetar un formato JSON estándar ApiError, con al menos:
...
2. El módulo exception-starter provee:
...
1. Acciones sensibles o relevantes (aprobaciones, rechazos, actualizaciones críticas) deben registrar un evento de auditoría mediante AuditLogger:
  - Ejemplos:
    - Aprobación de una solicitud de crédito.
    - Cambio de estado de una campaña.
    - Acceso a datos de cliente en contextos regulados.

2. Cada AuditEvent debe contener al menos:
...
3. Los equipos no deben escribir logs de auditoría “a mano” usando Logger.getLogger("AUDIT"); deben usar AuditEvent + AuditLogger para asegurar formato y campos mínimos.
...
**Lineamientos**
1. Toda llamada HTTP saliente crítica (a core, terceros, partners, etc.) debe cumplir:
    1. Estar encapsulada en un adaptador de infraestructura, por ejemplo:
      - infrastructure/proxy/<sistema>
      - implementando un puerto declarado en application/port/out.
      - Ejemplo:  ExternalUsersHttpClientAdapter implements ExternalUsersPort.
    2. Declarar una política de resiliencia mediante la anotación:
      - @ResilientHttpOutbound(<POLICY>)
    3. Usar obligatoriamente componentes estándar provistos por el framework:
      - @Timeout
      - @Retry (solo si el servicio externo es idempotente)
      - @CircuitBreaker
      - @Fallback (local o corporativo)
    4. Está prohibido implementar:
      - timeouts manuales,
      - retries explícitos,
      - circuit breakers,
...
2. Catálogo corporativo de políticas de resiliencia
...
3. Configuración declarativa desde application.properties
...
4. Configuración efectiva de MP Fault Tolerance
...
1. Overrides por método
2. Policies declarativas (resilience.policies.*)
3. Defaults globales MP FT

5. Aplicación en runtime
...
1. Presencia de puertos Application Outbound.
2. Uso obligatorio de políticas sobre adaptadores HTTP.
3. Ubicación correcta de invocaciones outboun
...
**Lineamientos**
1. Máximos globales (referenciales)
  - MP FT (síncrono HTTP):
...
**Lineamientos**
1. Pirámide de pruebas obligatoria
...
3. Tipos de pruebas obligatorias por servicio
  - REST:
...
1. Separación clara de tipos de test
...
2. Buenas prácticas específicas con Quarkus
...
4. Ejemplo de test unitario correcto (sin QuarkusTest, sin red)
...
**Lineamientos**
1. Etapas mínimas de pipeline (por microservicio)
  1. Build & Unit tests
  ...
  5. Build & Scan de imagen de contenedor
  ...
  7. Pruebas de integración / contract tests
    - Tests con Testcontainers.
    - Contract tests REST/EDA.
  8. Despliegue a DEV + smoke tests
    - Health checks + endpoints críticos.
  9. Promoción a QA/PROD
...
1. Mantener los tests rápidos y confiables
...
2. Evitar dependencias externas en los tests
  - Nunca depender de:
...
3. Usar ejemplos alineados al servicio real
...
4. Facilitar el trabajo del desarrollador
...
**Lineamientos**
1. OpenAPI obligatorio (source of truth)
...
2. Estructura mínima por endpoint
...
3. Uso obligatorio de modelos tipados
...
8. Headers corporativos obligatorios
...
3. Naming estándar para topics/colas
...
5. Metadatos mínimos en headers
...
7. Contrato AsyncAPI gestionado como archivo de proyecto
  - Se debe ubicar en:
...
8. Exposición estándar en runtime
...
9. Flujo corporativo para eventos
...
El pipeline corporativo realiza:
  1. Extracción automática del contrato
     Ejecuta un curl o wget al endpoint /q/openapi, por ejemplo:
       curl http://service-url/q/openapi -o openapi.yaml
	   
  2. Validación automática (linters y reglas corporativas)
    1. Spectral para OpenAPI ruleset corporativo.
    2. Validación sintáctica.
    3. Validación de versionado y cambios rompientes.
	
  3. Publicación en catálogos corporativos
    1. API Gateway / API Portal.
    2. Consola de documentación interna.
	
  4 Registro de auditoría
    1. Versión del contrato.
...
1. Uso obligatorio de linters para OpenAPI
...
1. Contract testing para APIs REST
...
2. Contract testing para eventos
...
Si una dependencia no está en el BOM:
  1. Se solicita vía ticket técnico.
  2. Arquitectura revisa seguridad, licenciamiento, CVEs, trazabilidad.
  3. Se agrega al BOM solo si cumple estándares corporativos.
...
El cumplimiento de los checklists no se valida manualmente “a ojo” en cada despliegue. El Framework de Integración define un esquema mixto de:
  ...
  2 .Revisiones técnicas periódicas (Architecture / Tech Review)
...
1. Separación clara de elementos DDD
...
2. Sin dependencias a frameworks
...
```java
  ...
  public class ActivateCampaignUseCaseImpl implements ActivateCampaignUseCase {
    ...
    public ActivateCampaignUseCaseImpl(CampaignRepositoryPort campaignRepository, DomainEventPublisherPort eventPublisher) {
        this.campaignRepository = campaignRepository;
        this.eventPublisher = eventPublisher;
    }
...
```java
  ...
  public class JpaCampaignRepositoryAdapter implements CampaignRepositoryPort {
    ...
    public JpaCampaignRepositoryAdapter(CampaignJpaMapper mapper, CampaignPanacheRepository delegate) {
      this.mapper = mapper;
...
```java
  ...
  public class Campaign {
    ...
    public Campaign(CampaignId id, CampaignStatus status, LocalDate startDate, LocalDate endDate) {
  ...
  class CampaignTest {
    ...
    void should_fail_to_activate_campaign_out_of_period() {
      ...
      IllegalStateException ex = assertThrows(
        IllegalStateException.class, () -> campaign.activate(today)
      );
...
Lo que hace el framework
  ...
  2. CampaignsResource.getCampaigns():
    - Escribe un log TECH:
      ```java
        {
          "loggerName": "TECH",
  ...
  public class RestClientHeadersPropagationFactory implements ClientHeadersFactory {
    ...
    MultivaluedMap<String, String> outgoing) {
      ...
      String corrId = firstNonBlank(
        incoming != null ? incoming.getFirst(CORR_ID) : null, (String) MDC.get("correlationId")
      );
      
      String bizId = firstNonBlank(
      incoming != null ? incoming.getFirst(BIZ_ID) : null, (String) MDC.get("businessId")
      );
...
public class GlobalExceptionMapper implements ExceptionMapper < Throwable > {
	...
	public Response toResponse(Throwable exception) {
		...
		if(exception instanceof ConstraintViolationException) {
			...
			body = new ApiError(ErrorCode.VALIDATION_ERROR.name(), "Error de validación en la solicitud.", cve.getMessage(), traceId, correlationId, businessId);
			LOG.warnf("Validation error at %s (traceId=%s, corrId=%s, bizId=%s): %s", safePath(), traceId, correlationId, businessId, cve.getMessage());
		}
		// DOWNSTREAM (si resilience-starter está presente)
		else if(isResilienceException(exception)) {
			status = Response.Status.BAD_GATEWAY;
			body = new ApiError(ErrorCode.DOWNSTREAM_ERROR.name(), "Error al invocar un servicio externo.", exception.getMessage(), traceId, correlationId, businessId);
			LOG.errorf(exception, "Downstream failure at %s (traceId=%s, corrId=%s, bizId=%s)", safePath(), traceId, correlationId, businessId);
		} else {
			status = Response.Status.INTERNAL_SERVER_ERROR;
			body = new ApiError(ErrorCode.TECHNICAL_ERROR.name(), "Ocurrió un error interno al procesar la operación.", exception.getMessage(), traceId, correlationId, businessId);
			LOG.errorf(exception, "Unhandled error at %s (traceId=%s, corrId=%s, bizId=%s)", safePath(), traceId, correlationId, businessId);
		}
...
public class GetCampaignsUseCase {
	...
	@PostConstruct
	void initMetrics() {
		this.campaignsQueriesTotal = Counter.builder("campaigns_queries_total").description("Número de ejecuciones del caso de uso GetCampaigns").tag("usecase", "get_campaigns").register(registry);
		Gauge.builder("campaigns_last_result_size", campaignsLastResultSize, AtomicInteger::get).description("Tamaño del último resultado de campañas").tag("usecase", "get_campaigns").register(registry);
	}
	public List < Campaign > execute() {
		...
		try (Scope scope = span.makeCurrent()) {
			...
			List < Campaign > campaigns = List.of(new Campaign("CMP-001", "Campaña de ahorros"), new Campaign("CMP-002", "Campaña de créditos"));
			...
			BusinessLogger.info("GET_CAMPAIGNS", "Consulta de campañas ejecutada", Map.of("count", campaigns.size()));
			AuditLogger.log(AuditEvent.builder("GET_CAMPAIGNS").entity("Campaign").outcome("SUCCESS").metadata(Map.of("count", campaigns.size())).build());
...
public class TestLoggingCampaignResource {
	...
	@GET
	@Path("/basic")
	public Response basicLogging(@HeaderParam("X-Correlation-Id") String corrHeader, @HeaderParam("X-Business-Id") String bizHeader) {
	...
	@GET
	@Path("/audit-success")
	public Response auditSuccess(@HeaderParam("X-Correlation-Id") String corrHeader, @HeaderParam("X-Business-Id") String bizHeader) {
		String corr = corrHeader != null ? corrHeader : (String) MDC.get("correlationId");
		String biz = bizHeader != null ? bizHeader : (String) MDC.get("businessId");
		LOG.infof("TestLoggingCampaignResource.auditSuccess() invocado (corrId=%s, bizId=%s)", corr, biz);
		AuditEvent event = AuditEvent.builder("CAMPAIGN_ACTION").entity("CAMPAIGN").outcome("SUCCESS").correlationId(corr)
			// si tienes reason opcional, puedes dejarlo como null,
			// pero eso ya lo maneja internamente el builder
			.metadata(Map.of("operation", "CAMPAIGN_ACTION", "source", "TestLoggingCampaignResource", "demo", true)).build();
	...
	@GET
	@Path("/audit-failure")
	public Response auditFailure(@HeaderParam("X-Correlation-Id") String corrHeader, @HeaderParam("X-Business-Id") String bizHeader) {
		String corr = corrHeader != null ? corrHeader : (String) MDC.get("correlationId");
		String biz = bizHeader != null ? bizHeader : (String) MDC.get("businessId");
		...
		AuditEvent event = AuditEvent.builder("CAMPAIGN_ACTION").entity("CAMPAIGN").outcome("FAILURE").correlationId(corr).reason(reason).metadata(Map.of("operation", "CAMPAIGN_ACTION", "source", "TestLoggingCampaignResource", "demo", true)).build();
		AuditLogger.log(event);
		Map < String, Object > body = new HashMap < > ();
...
DEBUG ResilientHttpOutbound policy='external-users' applied to GetExternalUsersService#listUsers
...
**Cómo aplica los lineamientos:**
  ...
  - Generación del contrato
    ...
    - Este documento es utilizado posteriormente por:
      1. Pipeline de CI/CD, que:
        - descarga /q/openapi,
        - valida el archivo usando Spectral y reglas corporativas,
        - ejecuta contract testing.
      2. API Portal corporativo, donde el contrato aprobado se publica automáticamente.
      3. Equipos consumidores, que lo usan como fuente única de especificación funcional y técnica.
